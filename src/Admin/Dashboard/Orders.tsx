  ...
  return (
    ...
    >
      <div className="h-36 bg-gray-800/40 flex items-center justify-center border-b border-gray-800/60">
        <img src={firstImage} alt="Order" className="h-full w-auto object-contain p-3" />
      </div>

      <div className="p-4">
        <p className="text-[10px] text-gray-600 font-roboto font-mono truncate mb-2">ID: {order.id}</p>
...
const Orders: React.FC = () => {
  ...
  const [lookupOrder, setLookupOrder] = useState<any | null>(null);
  ...
  const runIdLookup = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    let found = orders.find(
      (o) => String(o.id) === q || String(o.id).toLowerCase() === q.toLowerCase()
    );
    if (!found) {
      const { data, error } = await supabase.from("order").select("*").eq("id", q).maybeSingle();
      if (!error && data) found = data;
    }
    setLookupOrder(found ?? null);
  ...
  const openOrder = (id: string | number) => navigate(`${ROUTES.ADMIN_ORDER_DETAILS}/${id}`);
    ...
    <div className="w-full">
      ...
      <div className="px-4 md:px-8 py-6">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
          <h2 className="text-xl md:text-2xl text-gray-100 font-dayone">Purchase Requests</h2>

          <form onSubmit={runIdLookup} className="flex flex-col sm:flex-row gap-2 w-full md:w-auto md:min-w-[320px]">
            <input
              type="text"
              value={idQuery}
              onChange={(e) => setIdQuery(e.target.value)}
              placeholder="Paste order ID…"
              className="flex-1 px-3 py-2 text-sm font-roboto text-gray-100 bg-gray-900 border border-gray-800 rounded-lg outline-none focus:border-Storepurple/60 focus:ring-1 focus:ring-Storepurple/30 placeholder:text-gray-600 font-mono"
              autoComplete="off"
            />
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-2 text-sm font-roboto font-bold text-white bg-Storepurple hover:bg-StorepurpleDark rounded-lg transition-colors disabled:opacity-60"
              >
                {lookupLoading ? "…" : "Find"}
              </button>
              {(lookupTried || idQuery) && (
                <button
                  type="button"
                  onClick={clearLookup}
                  className="px-3 py-2 text-sm font-roboto text-gray-400 hover:text-gray-200 border border-gray-800 rounded-lg"
                >
                  Clear
                </button>
              )}
        ...
        {lookupTried && !lookupLoading && (
          <div className="mb-8">
            {lookupOrder ? (
              <div>
                <p className="text-xs font-roboto font-bold uppercase tracking-widest text-gray-500 mb-3">
                  Order lookup
                </p>
                <div className="max-w-md">
                  <OrderCard order={lookupOrder} priceFormat={priceFormat} onOpen={openOrder} />
                </div>
              </div>
            ) : (
              <div className="rounded-xl border border-gray-800 bg-gray-900/50 px-4 py-3">
                <p className="text-sm text-gray-400 font-roboto">No order found with that ID.</p>
              </div>
            )}
          </div>
        )}
        ...
        {!loading && (
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((order: any) => (
              <OrderCard key={order.id} order={order} priceFormat={priceFormat} onOpen={openOrder} />
            ))}
          </div>
        )}
      </div>
